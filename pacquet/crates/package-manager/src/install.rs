    ...
    DependencyGroupList: IntoIterator<Item = DependencyGroup>,
{
    ...
    pub async fn run<Reporter: self::Reporter + 'static>(self) -> Result<(), InstallError> {
        ...
        // before it proceeds. `trust_lockfile` or a missing lockfile leaves
        // the list empty, making every gate a no-op — fresh local
        // resolution is already filtered by the resolver's own per-version
        // gate. Mirrors pnpm's wiring at
        // <https://github.com/pnpm/pnpm/blob/2a9bd897bf/installing/deps-installer/src/install/index.ts#L355-L383>
        // and its concurrent verification + build gate.
        //
        // [#11860]: <https://github.com/pnpm/pnpm/issues/11860>
        let resolution_verifiers = if lockfile.is_some() && !trust_lockfile {
